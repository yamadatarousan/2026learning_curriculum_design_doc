...
// FindAllは、指定されたユーザーIDに紐づくすべてのTODOアイテムを取得します。
...
// --- Day 46: User authentication methods ---

func (r *TodoRepository) CreateUser(user User) (User, error) {
	err := r.db.QueryRow("INSERT INTO users (email, password_hash) VALUES ($1, $2) RETURNING id, created_at", user.Email, user.PasswordHash).Scan(&user.ID, &user.CreatedAt)
	if err != nil {
		return user, err
	}
	return user, nil
