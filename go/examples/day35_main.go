...
// Nameフィールドにバリデーションルール `binding:"required"` を追加します。
type Todo struct {
	ID   int    `json:"id"`
...
func initDB() {
	var err error
	db, err = sql.Open("sqlite3", "./todos_day35.db")
	if err != nil {
...
func createTodo(c *gin.Context) {
	var newTodo Todo
	// c.BindJSON はリクエストボディのJSONを構造体にバインドします。
	// この時、構造体タグに定義されたバリデーションルール（binding:"required"）も同時にチェックされます。
	if err := c.BindJSON(&newTodo); err != nil {
		// バリデーションに失敗した場合、エラーの詳細をクライアントに返します。
