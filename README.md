...
本APIにおける、役割ごとのアクセス制御は以下の通りです。

| エンドポイント                  | HTTPメソッド | 対象ユーザー        | 説明                                   |
| ------------------------------- | ------------ | ------------------- | -------------------------------------- |
| `/signup`                       | `POST`       | **誰でも** (Public) | 新規ユーザー登録                       |
| `/login`                        | `POST`       | **誰でも** (Public) | ログインしてJWTを取得                  |
| `/api/v1/todos`                 | `GET`        | **認証済みユーザー**  | 自身のTODOリストを取得                 |
| `/api/v1/todos`                 | `POST`       | **認証済みユーザー**  | 新しいTODOを作成                       |
| `/api/v1/admin/users`           | `GET`        | **管理者 (admin)**  | 全ユーザーのリストを取得               |